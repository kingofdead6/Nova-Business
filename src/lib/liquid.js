...
/* -------------------------------------------------------------------------- */
/*
 * Le semis d'étoiles qui occupe le ciel, en coordonnées du viewBox.
 *
 * Il est CONSTANT : généré une fois au chargement du module à partir d'une
 * graine fixe, jamais recalculé. Le ciel est donc le même d'une visite à
 * l'autre, et le rendu ne coûte que la création des nœuds, une seule fois.
 *
 * Trois grandeurs, comme sur une planche d'astronomie : quelques étoiles
 * franches, davantage de moyennes, un semis fin pour la profondeur. C'est le
 * rapport entre les trois qui donne l'échelle, pas un flou d'ambiance.
 *
 * `r` est exprimé en unités de viewBox HORIZONTALES. Le SVG étant étiré
 * (`preserveAspectRatio="none"`), on dessine des <ellipse> dont le rayon
 * vertical est corrigé à l'affichage — voir LiquidVeil.
 */
const MAGNITUDES = [
  { count: 26, r: [2.6, 4.2], a: [0.72, 1] },
  { count: 68, r: [1.6, 2.6], a: [0.45, 0.75] },
  { count: 130, r: [0.9, 1.6], a: [0.22, 0.48] },
];

export const SKY_STARS = (() => {
  /* même générateur que le reste du module : reproductible, sans dépendance */
  let s = 20260907;
  const rand = () => {
  ...
  for (const m of MAGNITUDES) {
    for (let i = 0; i < m.count; i += 1) {
      out.push({
        x: +(rand() * W).toFixed(1),
        y: +(rand() * H).toFixed(1),
        r: +(m.r[0] + rand() * (m.r[1] - m.r[0])).toFixed(2),
        a: +(m.a[0] + rand() * (m.a[1] - m.a[0])).toFixed(2),
      });
    }
  }
  return out;
...
export const LAYERS = [
  {
    id: "pale",
    color: "#D6D2E0", // clair bleuté : l'avant-garde de la coulée
    seed: 1.7,
    amp: 158,
    nodes: 96,
    tension: 1.22,
  ...
  {
    id: "mid",
    color: "#4A5280", // valeur intermédiaire : donne l'épaisseur
    seed: 4.2,
    amp: 142,
    nodes: 104,
    tension: 1.16,
  ...
  {
    id: "deep",
    color: "#171A2E", // la couche dominante : c'est elle qui recouvre tout
    seed: 7.9,
    amp: 126,
    nodes: 112,
    tension: 1.1,
...
export const PALETTES = {
  dark: ["#D6D2E0", "#4A5280", "#171A2E"],
  light: ["#2A2E48", "#7C86A8", "#F1ECE0"],
};
